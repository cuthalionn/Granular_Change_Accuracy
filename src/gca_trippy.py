...
class ModelResult():
    ...
    def __init__(self,all_turn_beliefs,
                all_preds_beliefs,
                wrong_preds,
                total_turn,
                total_pred,
    ...
    for dial_key in data.keys():
        ...
        for turn_id in data[dial_key].keys():
            ...
            for label in turn_dif:
                ...
                elif trippy_correct(pred_belief[label],turn_dif[label]):
                    dial_correct += 1
                    correct_labels.add(label)
            ...
            for label in pred_dif:
                if label not in turn_belief and pred_dif[label] != "none":
                    ...
                    dial_correct += 1 
                elif trippy_correct(pred_dif[label],turn_belief[label]) and label not in correct_labels:
                    dial_correct += 1
...
def main():
    ...
    for data_path in eval_file_dir:
        ...
        _,_,_, model_result = compute_gca_trippy(data)
        print(model_result)
